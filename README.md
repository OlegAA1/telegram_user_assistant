...
- Если **`ASK_SENDER_IDS`** пуст и не задан устаревший **`OWNER_ID`** — `/ask` **выключен**.
...
1. В логах при старте должно быть **`/ask and /remind enabled for sender user ids: [...]`**. Если видно **`ASK_SENDER_IDS ... is disabled`** — в `.env` пустой список и не задан **`OWNER_ID`**.
2. В **`ASK_SENDER_IDS`** должен быть **ровно тот numeric user id**, с которого ты пишешь (часто это не username). Проверь через `@userinfobot` с **того же аккаунта**, с которого шлёшь `/ask`.
...
Тем же списком разрешённых отправителей, что и для `/ask`, в личку ассистенту можно ставить напоминания. Время **абсолютных** напоминаний задаётся в часовом поясе **`REMINDER_TZ`** (IANA, например `Europe/Moscow`).
...
- Свободную формулировку («напомни завтра утром») можно сначала прогнать через **`/ask`**, чтобы получить конкретную дату/время, затем оформить **`/remind ...`** вручную (парсер пока не «читает» русский текст без шаблона).
